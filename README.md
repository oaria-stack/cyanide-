...
- **Atria Lite**: tethered Atria-inspired layout controls for dock count,
  home-screen rows/columns, labels, loaded-icon scale, and icon offset.
...
- **SnowBoard Lite**: applies tethered custom PNG icons from
  `Cyanide/SnowBoardLiteTheme`, using bundle identifiers as filenames.

SnowBoard Lite theme files are bundled at build time. Add PNGs to
`Cyanide/SnowBoardLiteTheme` with bundle-ID filenames, for example
`com.apple.MobileSMS.png` or `com.apple.Preferences.png`, then rebuild the IPA.
Apply briefly reveals App Library to load more SpringBoard icon views, swaps
matching icons in memory, and returns to the home pages when a supported private
selector is available. The theme resets after SpringBoard reloads, respring, or
reboot.
...
You can also use the GitHub Actions-friendly wrapper:

```sh
bash ./buildipa.sh
```

The `Build IPA` workflow runs that same wrapper and uploads the IPA as a
